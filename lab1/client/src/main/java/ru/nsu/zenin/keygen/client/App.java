...
public class App {
    ...
    private static ClientConfig parseArgs(CommandLine cmd) throws ParseException {
        ...
        String outputName = cmd.getOptionValue(App.outputName);

        String delayRaw = cmd.getOptionValue(delay);
        long delayArg = delayRaw == null ? 0 : Long.parseLong(delayRaw);

        Path outputDir;
        if (cmd.hasOption(App.outputDir)) {
            outputDir = Paths.get(cmd.getOptionValue(App.outputDir));
        } else {
            outputDir = Paths.get(".");
        }
    ...
    private static void appMain(ClientConfig conf)
        ...
        Client client = new Client(conf.getEndpoint(), conf.getFail(), conf.getDelay());
        if (conf.getFail()) {
            client.getKeypairAndCert(conf.getName());
            return;
        }

        KeypairAndCert keypairAndCert = client.getKeypairAndCert(conf.getName()).get();
