...
public class Client {
  ...
  private static Writer tryCreateFile(Path file) throws IOException {
    if (Files.exists(file)) {
      ...
      while (true) {
        System.out.print("File " + file.toString() + " will be overwritten, continue? (y/n) ");
        ...
        try {
          ...
          if (line.equals("y")) {
            ...
            throw new IOException(file.toString() + ": Existing file cannot be overwritten");
          }
  ...
  private static void appMain(InetSocketAddress addr, String subjectName, long delay, boolean fail)
      throws IOException, InterruptedException, InvalidKeySpecException, NoSuchAlgorithmException {
    try (Socket sock = new Socket()) {
      ...
      try {
        ...
        try (PemWriter publicKeyWriter = new PemWriter(tryCreateFile(publicKeyFile))) {
          try (PemWriter privateKeyWriter = new PemWriter(tryCreateFile(privateKeyFile))) {
            try (PemWriter certWriter = new PemWriter(tryCreateFile(certFile))) {
              KeyFactory keyFactory = KeyFactory.getInstance(keypairAndCert.getAlgorithm());
              ...
              certWriter.writeObject(
                  new PemObject("CERTIFICATE", keypairAndCert.getCert().getEncoded()));
            }
          }
        }
